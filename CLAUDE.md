...
**타로 리딩 max_tokens 정책**: `computeReadingMaxTokens(cardCount)` — 1장→2000, 3장→3500, 5장→5000, 7장→6500, 9장→8000, 10장→11000(celtic-cross), 12장+→13000(zodiac 등). 한국어 토큰 효율 영어 대비 1.3배 낮음 반영. `parseError === "invalid_json"` 시 에러 화면, `"truncated"` 시 경고 메시지 후 결과 화면 진입(부분 결과 표시).
