...
const DIRECT_ANSWER_EVIDENCE: Record<DirectAnswerDomain, string> = {
  tarot: "뽑힌 카드의 그림과 자리",
  saju: "올해·이달 운세 흐름 같은 사주 근거 — 세운·간지 등 사주 용어는 쓰는 즉시 쉬운 말로 풀어서(예: 세운→올해 운, 병오화→불 기운이 센 해)",
...
- 옆자리 친구에게 말하듯 따뜻하게, 위에서 정한 캐릭터의 말투(어미)를 처음부터 끝까지 일관되게 지킵니다. 카드·상황·구조·운세를 묘사할 때도 설명하듯 '~다/~이다/~한다'로 바꾸지 말고, 상담자에게 직접 말 걸듯 캐릭터의 말투로 문장을 끝맺습니다(감탄사 남발·이모지 없이 품위 있게).
...
- 다 쓴 뒤 스스로 읽어, 사람이 실제로 말하지 않는 번역기 말투('~에 다름 아니다', '~라 할 수 있을 것이다', '보여진다·되어진다' 같은 이중피동)와 중간에 섞인 설명체('~다/~이다')를 캐릭터의 말투로 고쳐 최종본만 씁니다.
