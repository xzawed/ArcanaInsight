...
interface SajuResultShareButtonProps {
  shareToken: string;
}

export function SajuResultShareButton({ shareToken }: SajuResultShareButtonProps) {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: "사주 분석 결과 - ArcanaInsight", text, url });
      } catch {
        /* 사용자가 공유 취소 */
      }
    } else {
