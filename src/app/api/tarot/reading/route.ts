...
import { saveTarotReading, logReadingSaveFailure, recordFailedReading, persistDirectAnswer, logReadingParseError, recordParseFailure } from "@/lib/db/reading-saver";
      ...
      async start(controller) {
        try {
          ...
          if (result.parseError) {
            logReadingParseError("tarot", result.parseError, sessionId ?? null);
            if (db) await recordParseFailure(db, "tarot", result.parseError, sessionId ?? null, locale);
          }
