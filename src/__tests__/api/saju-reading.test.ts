...
describe("POST /api/saju/reading", () => {
  ...
  it("스트림 완료 후 saveSajuReading 호출 (done 이후 await)", async () => {
    ...
    vi.doMock("@/lib/db/reading-saver", () => ({ persistDirectAnswer: vi.fn(), saveSajuReading: mockSave, logReadingParseError: vi.fn(), recordParseFailure: vi.fn() }));
    vi.doMock("@/lib/rate-limit", () => ({
      checkRateLimit: vi.fn().mockReturnValue(true),
      rateLimitResponse: vi.fn(),
    }));
  ...
  it("저장 성공 시 done 이후 saved:true 이벤트를 전송한다", async () => {
    const mockSave = vi.fn().mockResolvedValue(undefined);
    vi.doMock("@/lib/db/reading-saver", () => ({ persistDirectAnswer: vi.fn(), saveSajuReading: mockSave, logReadingSaveFailure: vi.fn(), logReadingParseError: vi.fn(), recordParseFailure: vi.fn() }));
  ...
  it("저장 실패 시 saved:false 이벤트 전송 + logReadingSaveFailure 호출", async () => {
    ...
    vi.doMock("@/lib/db/reading-saver", () => ({ persistDirectAnswer: vi.fn(), saveSajuReading: mockSave, logReadingSaveFailure: mockLog, recordFailedReading: mockRecord, logReadingParseError: vi.fn(), recordParseFailure: vi.fn() }));
    vi.doMock("@/lib/rate-limit", () => ({ checkRateLimit: vi.fn().mockReturnValue(true), rateLimitResponse: vi.fn() }));
    const mockDb = makeMockDb();
    vi.doMock("@/lib/db", () => ({ getDb: vi.fn().mockReturnValue(mockDb), getAdminDb: vi.fn().mockReturnValue(mockDb) }));
    vi.doMock("@/lib/auth", () => makeAuthMock());
