...
| 항목 | 파일 | 현황 | 해결 조건 | 담당 |
|------|------|------|----------|------|
| 커버리지 측정 범위 협소 | `vitest.config.ts` coverage.include | whitelist 방식. **B-4 점진 확장(2026-06-30)**: `time-utils`·`guest-sessions`·`reading-saver` 3파일 추가(측정 statements 1450→1525, 임계값 불변 유지). 잔여 후보 `character-context`·`postgres-adapter`는 테스트 갭으로 statements<98% 유발 → 테스트 보강 후 추가 가능. | 미커버 후보 테스트 보강 후 추가, 또는 exclude 방식 전환 | Claude |
