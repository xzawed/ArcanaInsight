...
| E2E "Target closed" 크래시 (DC·MA·iOS) — **원인 정정 (2026-07-31)** | `playwright.config.ts`, `.github/workflows/deploy.yml` | **🔬 OOM 가설 반증 (2026-07-31, #522 → PR #524)**: 6런(24 E2E 잡) 1초 간격 계측으로 **메모리 원인은 배제됐다.** 러너 실측은 `nproc=4` · `15989 MiB`(AMD EPYC 7763/9V74)로 #462가 든 **"2코어/7GB"는 코어 수까지 틀렸다.** E2E 구간 메모리 피크 **2.4~3.4GiB(15~21%)**, 최저 available **12.4~13.6GiB**, **swap 0**, **dmesg OOM 흔적 0** — 전 잡 예외 없음. 결정타로 **`expect.toBeVisible: Target page, context or browser has been closed`가 `workers:1` · available 13136 MiB · swap 0 · OOM 흔적 없음 상태에서 재현**됐다(Mobile Android 2/2, run 30626475036). ∴ **이 시그니처는 OOM의 증거가 아니다.** 실제로 압박받는 자원은 **CPU**다 — `workers:1`에서도 평균 busy **63~75%**(iowait 0.0~0.2%로 I/O 대기 아님)·버스트 100%. `workers:2` 시험(Desktop Chrome 3런)은 테스트 시간을 2.9~3.9m → **2.0~2.9m**로 줄였으나 평균 busy가 **83~95%**로 올라 30s 타임아웃 여유가 얇아졌고, **E2E 임계경로가 Mobile Android(5.0~6.3m)라 CI 벽시계 이득이 0**이라 `workers:1`로 되돌렸다. 계측(`nproc`·피크 메모리·CPU busy%·loadavg·OOM 흔적)은 잡 요약에 상설로 남는다. ⚠️ 아래 2026-07-29·07-04 기록의 **"OOM"·"OOM-killer" 서술은 이 반증으로 폐기**됐다(관측 사실인 "비결정적·코드 무관"은 유효). 아래는 이력 보존용이다. **⚠️ 재발 기록 (2026-07-29, PR #509 CI)**: `workers:1` 적용 후에도 동일 시그니처 재현. E2E Desktop Chrome이 **21건 실패·37.2m**(정상 대비 5.4배) — `Test timeout of 30000ms exceeded` / `Target page, context or browser has been closed` / `page.goto: net::ERR_ABORTED; maybe frame was detached?`. 실패 분포는 navigation 7·immersive-layout 3·theme 2·shinjeom-flow 2 등 특정 spec 무관하게 산발. **동일 커밋 무변경 재실행 → 통과(6m53s)** ⇒ 비결정적 확정. 같은 커밋 Mobile Android는 정상(179개 9.9m), 직전 #508 런은 Desktop Chrome 180개 7.8m 통과. 배제한 원인: PR 변경분(문서·`.gitignore`·eslint 설정·devDep 버전·`onlyBuiltDependencies` — CI 설치 로그에 build script 차단 경고 0·477패키지 정상), sharp WASM 폴백(`@img/sharp-linux-x64@0.35.3` 정상 존재, emnapi 중복 정리는 wasm32 전용), 병합상태 CI 미검증(#506 15:02Z 머지 → #508 CI 15:28Z 시작이라 검증됨). ⚠️ 이번 건은 메모리 계측을 붙이지 않아 **OOM은 시그니처 일치 기반 추정**이며 직접 관측된 것은 "비결정적·코드 무관"까지다. **운영 지침: 이 시그니처(+평소 7~11분의 3배 이상 소요)에 한해 동일 커밋 재실행(`gh run rerun <id> --failed`)으로 재현성을 확인한다. 재실행 통과는 진단 결과일 뿐 해결이 아니며, 확인된 flake는 넘어갈 신호가 아니라 이 표에 재발을 기록할 인프라 결함이다.** 단언 실패(깨진 이미지·콘솔 에러 등)는 이 경로 대상이 아니다 — 재시도로 통과해도 실제 결함일 수 있다(2026-07-29 실증: 같은 런에서 가드가 `placeholder.supabase.co` 카드 이미지 404를 정확히 탐지했으나 `retries:2`가 `1 flaky`로 흡수해 green 처리). `workers:1`은 빈도를 낮췄을 뿐 근본 해소가 아니다. 아래는 2026-07-04 최초 규명 기록. **근본원인 규명·수정 (2026-07-04, #462)** — 만성 "Target page/context/browser has been closed"(Weekly QA #461)는 테스트 버그가 아니라 **호스트 OOM**: `workers:2`+`fullyParallel`이 브라우저 2개 + `pnpm start` Next 서버 + cold-cache sharp 이미지 최적화(원본 2816×1536 84장 → 16.5MiB 비트맵 디코드)를 한 2코어/7GB ubuntu 러너에 공존 → OOM-killer가 브라우저 프로세스 kill. **chromium·webkit 양쪽 재현 = 엔진 무관** ∴ `--disable-dev-shm-usage`는 no-op·반증(Playwright 기본 적용·bare VM shm는 GB급·webkit 무시). 4-각도 진단 워크플로우로 확정. **수정: CI `workers: 2→1`**(chromium DC·MA 해소, #462). **webkit(iOS)은 workers:1으로도 무거운 홈 이미지 테스트에서 크래시**(webkit 메모리-취약) → `cross-platform:60`(홈 이미지 로드 검사)을 **Desktop Chrome 전용 스코프**(깨진 이미지는 엔진 무관) + `daily-card` webkit `_rsc` 프리페치 benign 에러 필터(PR #464, 이슈 #463). #465는 비로그인 `GET /api/profile/favorite-character`를 401→200(`{characterId:null}`)로 바꿔 몰입형 진입 콘솔 401 제거(POST는 401 유지). 부수 perf: 테마 아이콘 `unoptimized` 제거(653KB→webp). ⚠️ 캐릭터 이미지 다운스케일은 **재검토 후 보류** — `nukki-enhanced` 2816×1536은 오독이 아니라 **고DPI 큰 표시(캐릭터 상세 모바일 100vw·세션)를 위한 의도된 2x 보정본**(docs/conventions/image-assets.md). 1408로 낮추면 고DPI 표시 품질 저하 → 균일 품질 요구와 충돌. CI 메모리는 workers:1로 해소되므로 에셋 변경 불필요. | ✅ **해소 (2026-08-01)** — 잔여 flake는 산발이 아니라 `navigation` 스크롤 테스트 **하나**로 수렴했고(3표본 12잡 중 나머지 9잡 clean), trace로 원인을 확정해 수정했다(#530): `click()`이 내부적으로 "waiting for scheduled navigations to finish"에서 대기하는데, 홈 전이가 이미지 큐 포화로 커밋되지 않아 예산을 통째로 태웠다. 이후 **CPU 여유가 63~75% → 42~55%** 로 개선돼(hydration 재렌더 제거 효과) 임계경로인 Mobile Android를 `workers:2`로 상향, 테스트 5.0m→2.9m·4.2m→2.7m 단축·2런 flaky 0 확인(#531). 임계경로는 이제 Desktop Chrome이다 | Claude |
...
| Phase 2 (카드 스타일) | 스타일 선택 UI, `useCardStyleStore`, `CardFace`/`CardBack` styleId 지원 | ✅ 완료 | 코드 구현 완료. 이미지 없으면 SVG fallback 렌더링. (초기의 `CardStyleSelector` 컴포넌트는 설정 페이지 인라인으로 대체돼 2026-08-01 제거) |
| Phase 3 (카드 스타일) | 설정 페이지에 스타일 선택 통합, SkinGallery 연동, 테마 자동 매핑 표시 | ✅ 완료 | `/settings` 카드 스킨 섹션에 11개 버튼 통합 |
