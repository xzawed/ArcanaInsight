...
| E2E `load`/`networkidle` 대기 → web-first 점진 sweep | `e2e/**` | 외부 R2 배경/카드 이미지가 `window.load`·`networkidle`을 지연 → Mobile Android 플레이키(#121~#428 재발). **몰입형 전환 완료**: navigation 스크롤-리셋(#455), 공유 헬퍼 `service-navigation.ts`·`ui-quality.spec.ts` 몰입형(#457). **잔여(저위험)**: `(site)` 페이지 networkidle ~7건(로컬 자산) + `theme.spec.ts`(load 8건)·`cross-platform.spec.ts` 등. eslint `no-networkidle` 가드 미도입(수동 컨벤션). | 잔여 플레이키 발생 시 우선 web-first 전환, 또는 eslint-plugin-playwright 도입 | Claude |
