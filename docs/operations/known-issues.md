...
| ~~E2E "Target closed" 크래시 (DC·MA·iOS)~~ | `playwright.config.ts`, `public/images/characters/*` | **근본원인 규명·수정 (2026-07-04, #462)** — 만성 "Target page/context/browser has been closed"(Weekly QA #461)는 테스트 버그가 아니라 **호스트 OOM**: `workers:2`+`fullyParallel`이 브라우저 2개 + `pnpm start` Next 서버 + cold-cache sharp 이미지 최적화(원본 2816×1536 84장 → 16.5MiB 비트맵 디코드)를 한 2코어/7GB ubuntu 러너에 공존 → OOM-killer가 브라우저 프로세스 kill. **chromium·webkit 양쪽 재현 = 엔진 무관** ∴ `--disable-dev-shm-usage`는 no-op·반증(Playwright 기본 적용·bare VM shm는 GB급·webkit 무시). 4-각도 진단 워크플로우로 확정. **수정: CI `workers: 2→1`**(엔진 무관, webkit 포함 해결). 부수 perf: 테마 아이콘 `unoptimized` 제거(653KB→webp). | (심층 perf 후속) 캐릭터 nukki 84장 **2816×1536→1408×768 재출력**(문서 목표치, 283MB↓·sharp 스파이크 4x↓) — R2 재업로드·무드별 시각 QA 필요 | Claude |
