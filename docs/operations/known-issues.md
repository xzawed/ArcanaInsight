...
| 의존성 취약점 현황 (2026-07-05 보안 점검) | `pnpm-lock.yaml`·`package.json` overrides | **prod 의존성 0건**(`pnpm audit --prod` clean). **postcss XSS(<8.5.10, moderate) → `postcss@<8.5.10: ^8.5.10` override로 해소**(next·@tailwindcss/postcss 경로, build 검증). 잔여는 **dev 전용 3건(노출 없음)**: esbuild dev-server 2건(Dependabot #38 low + moderate — `tsx`/`drizzle-kit`/`@esbuild-kit` 경로, `--serve` 미사용, `@esbuild-kit`~0.18 고정으로 override 지양), brace-expansion ReDoS(moderate — `minimatch` 경유 eslint 툴링, 설치본 1.1.13은 advisory(<1.1.12) 범위 밖). ws·@babel/core·js-yaml은 기존 overrides로 해소. **Secret: 커밋된 `.env` 0·하드코딩 키 0·secret-scanning 알림 0. code-scanning 미활성(CodeQL 미설정).** | esbuild: 상류가 0.28.1+로 bump 시 자동 해소 | 보류(노출 없음) |
