...
### 1단계: 기획·스펙·설계
- 사용자 요청을 받아 스펙 작성·설계·뼈대 스캐폴딩 수행
- `fix/*`, `feat/*`, `docs/*`, `chore/*` 기능 브랜치 개설 (main 직접 push 금지)

### 2단계: 구현 + 로컬 검증
- 설계한 스캐폴딩을 이어받아 본문 구현
- 완료 후 검증 4종 전부 실행:
...
```
사용자
  └─ [1단계]: 기획·스펙·브랜치 개설·스캐폴딩
       └─ [2단계]: 본문 구현 + 로컬 검증 4종
            └─ [3단계]: 아키텍처·보안·규칙 검토
                 ├─ 문제 발견 → 2단계 재진입
                 └─ 통과 → [4단계]: 구현 커밋 + PR 생성·설명 작성
                      └─ CI 자동 검증 [5단계]
                           ├─ 실패 → 수정 후 2단계 재진입
                           └─ 통과 → [6단계]: 머지 판단
                                └─ Railway 자동 배포
