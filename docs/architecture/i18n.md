...
ArcanaInsight는 한국어(`ko`)·영어(`en`)·일본어(`ja`) 3개 locale을 지원한다. 팀 내부 직역 사전 정책(외부 번역가 미사용)으로 SSOT는 한국어이며, 영어·일본어는 부분 번역(`Partial<SharedKeys>`)을 허용해 미번역 키는 자동 ko fallback 한다.
...
Request → middleware (Supabase auth + locale 결정)
            ↓
       [x-locale 헤더 부착] + [ai_locale 쿠키 변경 필요 시 갱신]
            ↓
...
- 마스터 플랜: [`docs/superpowers/plans/archive/i18n-master-plan.md`](../superpowers/plans/archive/i18n-master-plan.md)
- 정합성 핫픽스 계획(완료): [`docs/superpowers/plans/archive/i18n-consistency-fix.md`](../superpowers/plans/archive/i18n-consistency-fix.md)
