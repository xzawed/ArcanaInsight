...
| ~~R-3~~ | ~~타로 `interpretation` 레거시 필드 제거~~ | — | **파기 확정 (2026-08-01)**. 해결 조건이던 "구포맷 데이터 소멸 대기"가 성립하지 않는다 — 리딩에 TTL·보존 정책이 없어 **영구 보관**이다. 조사 중 훨씬 큰 결함을 발견해 그쪽을 고쳤다(아래) |
...
> **R-3 조사가 실제로 찾아낸 것 (2026-08-01).** 레거시 필드 제거 가능성을 보다가 **반대 방향의 결함**을 발견했다 —
> 공유·마이페이지 결과 페이지(`app/(site)/tarot/result/[id]/page.tsx`)가 **3-섹션을 아예 읽지 않고** `interpretation`만 봤다.
> #414(2026-05-26) 이후 저장된 리딩에는 그 키가 없어 `cleanReadingText(undefined)`가 **서버 컴포넌트를 터뜨렸다.**
> 약 **67일**간 살아 있었고, E2E가 404 경로만 검사해 한 번도 잡히지 않았다. PR로 수정 완료.
