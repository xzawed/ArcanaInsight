...
| ID | 작업 | 규모 | 비고 |
|---|---|---|---|
| **R-4** | **`default.png` ↔ `idle.png` 중복 통합** | 소(코드) + 확인 필요(자산) | 12명 전원 **md5 동일한 중복**이고 변형 60쌍도 같다. 저장소 약 42MiB·R2 객체 72개가 순수 중복. **코드 쪽 정리는 완료**(모든 호출부가 `idle` 사용). 남은 것은 `smoke-prod.mjs`·E2E가 참조하는 `default.png`를 `idle`로 옮긴 뒤 자산을 지우는 일 — **CDN 라이브 자산 삭제라 사용자 확인 후 진행한다.** 삭제 자체는 퍼지 불필요(덮어쓰기가 아니라 제거) |
