...
- ✅ **배포 후 실측 eval (2026-07-05)**: 프로덕션에서 타로·사주 리딩을 실제 생성해 톤 확인. **가독성 개선 확증** — 타로 directAnswer answer-first(재진술→방향 단언→확신 수위→근거) 완벽, 카드 해석이 "월요일 아침 회의에서 상사가 내린 결정에 고개를 끄덕이면서도…" 같은 구체 일상 장면으로 착지, 원형/신화/추상 은유 소거. 사주 3단 착지 확증("일간 경금"→"하얀 금속처럼 단단하고 차가운 기운"), 해요체 일관. 잔여 폴리시(비차단): ① 타로 카드 해석이 해요체/문어체(~다) 혼용 경향, ② 사주 directAnswer는 뾰족한 질문엔 잘 채우나 모호한 개방형 질문엔 간헐 누락(관측 로그가 포착), ③ 사주 directAnswer 근거에 일부 용어(세운 간지 등) 미풀이.
- ✅ **폴리시 튜닝 + 프로덕션 병렬 재검증 (2026-07-05, PR #476)**: (1) 쉬운말 계약이 '정중한 해요체'를 하드코딩해 zero(~다체)·ren(하오체) 등과 충돌하던 것을 **캐릭터 말투(어미) 일관성 준수**로 교체 → 5개 프로덕션 샘플 재검증에서 arcana(해요체)·zero(~다체) 각자 어미 일관 확증(문어체 드리프트 소거). (2) buildSajuPrompt 분량 기준에 directAnswer '항상 채움' 명시. (3) 사주 directAnswer 근거 용어 풀이 예시 강화. **잔여(수용 가능)**: 매우 개방형 질문("이번 주 운 어때?")의 saju directAnswer는 여전히 빈값 경향 — 특정 답을 요구하지 않아 overall/sections가 답하고 UI가 게이트하므로 강제 시 중복만 늘어 추가 반복 미진행. directAnswer 근거 간지 풀이도 일부 잔존(본문 sections는 완전 풀이).
