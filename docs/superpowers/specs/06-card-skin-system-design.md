...
> - Grok 이미지 모델명: 실제 사용 모델은 **`grok-2-image`** (02번 스펙의 `grok-imagine-image-pro`와 통일)
> - 이미지 수량: 앞면 78장 + 뒷면 1장 = **총 79장** × 6스킨 = 474장
>   (마이너 아르카나는 고유 일러스트가 아닌 수트별 SVG 템플릿 방식과 병행 — `CardFace.tsx` 참조)
> - 홈 페이지 삽입 위치: DailyCard(5번) 뒤, StatsCounter(7번) 앞
