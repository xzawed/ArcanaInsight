...
- 클라이언트 타임아웃(240s)은 유지 — max_tokens 증가로 응답 시간이 길어질 수 있으나, 기존 설계 마진 내에서 처리 가능 <br>⚠️ **(2026-08-01 갱신)** 이 시점 기록이다. 현재 클라이언트 하드 타임아웃은 **280s**이며 서버 `AI_TIMEOUT_MS`가 240s다(의도된 40s 마진). 정본: `docs/architecture/ai-infrastructure.md`
