> **Status**: 구현 완료
> **Note**: 설계 시점(2026-03-30) 기준 문서. 구현 과정에서 아래 항목이 변경됨:
> - "3자 제공 동의" → 정확한 법률 표현은 **"제3자 제공 동의"** (개인정보보호법 제17조)
> - 생년월일 범위 1950~2010: 실제 코드에서 동적 범위 처리 여부는 `UserInfoForm.tsx` 확인
> - `SessionPhase`에 `"user-info"` 단계 없음 → 타로 페이지에서 별도 step으로 처리
