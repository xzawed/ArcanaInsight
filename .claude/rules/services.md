...
### 리딩 안정성 내성 (2026-07-06 방어 → 2026-07-07 근본 제거)

사주·신점 리딩의 간헐적 JSON 형식 위반(트레일링 콤마·flat 필드를 섹션 객체 내부에 중첩)이 무결과를 유발했다. 2026-07-06엔 파서·라우트에 방어를 추가했고, 2026-07-07엔 근본 원인인 `sajuSections`/`shinjeomSections` 섹션 스키마 자체를 제거해 `overallReading`을 정본으로 통합했다(리딩 신뢰성 기술부채 정리). 현재 유효한 내성:

1. **`parseJsonSafe`** — 3차 파싱 시도에 **트레일링 콤마 제거** 포함 (`,}`·`,]` → `JSON.parse` 실패 방지).
2. **`streamReadingWithParseRetry`** (`reading-generator.ts`) — 3개 리딩 라우트가 `for await streamReading` 대신 이 헬퍼 사용. 1차 파싱이 `parseError`면 **1회 non-stream 재생성** 후 재파싱(성공 시 채택, 실패 시 원본 유지).

> `promoteNestedFields`(섹션 내부 flat 필드 승격)는 승격 대상(섹션)이 사라져 함께 제거됨.
