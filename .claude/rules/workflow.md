...
| E2E `Target closed`·`ERR_ABORTED`·30s 타임아웃 다발 | **시그니처를 먼저 분류한다.** 위 3종 + 평소의 3배 이상 소요이면 비결정 실패 후보 → 동일 커밋 재실행(`gh run rerun <runId> --failed`)으로 **재현성만 확인**한다. ⚠️ 재실행 통과는 진단 결과일 뿐 **해결이 아니다.** ⚠️ **단언 실패(깨진 이미지·콘솔 에러)는 이 경로로 보내지 말 것** — 재시도로 통과해도 실제 결함일 수 있다. 계측·이력·폐기된 가설(OOM 가설은 **반증됨**)은 [`e2e-incidents.md`](../../docs/operations/e2e-incidents.md)가 정본이며 재발도 거기 기록한다 |
